...
# torch.save(best_model_state, 'bestGRUpth') - un comment to save
...
np.savetxt('GRU_MSE.txt', [meanLosses])
...
testGRU.load_state_dict(torch.load('bestGRU_BCE.pth'))
...
with torch.no_grad():
    ...
    for data in loadTest:
        ...
        for i in range(len(video)):
            ...
            #test_lossMSE += criterionMSE(predictionFinal, targetFinal).item()
            #test_lossMAE += criterionMAE(predictionFinal, targetFinal).item()
            #test_lossBCE += criterionBCE(predictionFinal, targetFinal).item()
...
plt.title('Video Wise Losses - BCE Metric - GRU prediction')
plt.xlabel('Video ID')
plt.ylabel('Losses (%)')
plt.grid()
