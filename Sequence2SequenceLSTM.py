...
#LSTM Class for sequence to sequence prediction
class LSTM(nn.Module):
    def __init__(self):
        super(LSTM, self).__init__()

        self.rnn = nn.LSTM(input_size=16, hidden_size=16, batch_first=True)
    ...
    def forward(self, x):
        ...
        lstmx, _ = self.rnn(flatx) # run through the LSTM layer
...
# Initialising the CAE, and Seq2Seq Objects whcih are pytorch nn.modules
predictor = LSTM().to(device)
...
# Training the model wher 12 frames are used as basis to predict the remaining 4
meanLosses = []
epochNumber = []

start_time = time.time()

for epoch in range(epochs): # Epoch starts

    cumuLoss = 0.0

    for data in loadTrain: # 3 batchs of 11 videos are loaded, 1 is chosen

        video, _ = data
        allPred = torch.tensor([], dtype = torch.float32)
        allTargs = torch.tensor([], dtype = torch.float32)

        for i in range(len(video)): # iterating through video list, to pick out 1 video

            inputs = video[i].unsqueeze(1).to(device) # add information about grayscale channel necessary

            basis = inputs[:12] # basis 12 frames that are used for prediction
            targets = inputs[12:] # target 4 frames (remaining ones for the video)
            optimizer.zero_grad() # training 
            
            basisCompression, _, _ = encoder.compress(basis) # compress the basis frames         
            targetsCompression, sizeT, indexT = encoder.compress(targets) # compress the target frames

            outputs = predictor.forward(basisCompression) # take the 12 frames, encode, predict, and decode
            predictionFinal = outputs[:4] # only need the first 4 frames as the rest are outisde video bounds
            
            targetFinal = encoder.decompress(targetsCompression, indexT, sizeT) # take target frames encode decode to simplify training

            allPred = torch.cat((allPred, predictionFinal), dim = 0)
            allTargs = torch.cat((allTargs, targetFinal), dim = 0)

        loss = criterion(allPred, allTargs)
    ...
    with torch.no_grad(): # validation set
        ...
        for data in loadValid: # choose a batch

            video, _ = data # load the videos from the chosen batch

            for i in range(len(video)): # iterate through the videos for chosen batch
                inputs = video[i].unsqueeze(1).to(device) # unsqueeze for binary

                basis = inputs[:12] # basis 12 frames
                targets = inputs[12:] # target final 4 frames
                
                basisCompression, _, _ = encoder.compress(basis) # compress the basis frames         
                targetsCompression, sizeT, indexT = encoder.compress(targets) # compress the target frames

                outputs = predictor.forward(basisCompression) # parse and predict next 12 frames 
                predictionFinal = outputs[:4] # only need the first 4 frames as the rest are out of bounds
                
                targetFinal = encoder.decompress(targetsCompression, indexT, sizeT) # encode and decode the target 4 frames

                allPred = torch.cat((allPred, predictionFinal), dim = 0)
                allTargs = torch.cat((allTargs, targetFinal), dim = 0)
        
            val_loss = criterion(allPred, allTargs)  # Calculates loss of the whole batch    
            cumuLoss += val_loss # add up losses of all batches

        meanVal_loss = cumuLoss/batch # mean loss of all batches

        meanLosses.append(meanVal_loss)
        epochNumber.append(epoch)

    # conditions for storing model information and loss information
...
#torch.save(best_model_state, 'bestLSTM_MSE.pth')

end_time = time.time()

elapsed_time = end_time - start_time

print(f"Elapsed time for training of LSTM Method: {elapsed_time} seconds")

# %%

#plotting out the evoluation of the loss function MSE as the model gets trained
...
plt.plot(epochNumber, meanLosses * 100, 'x-', color = 'black', 
         label ='MSE Loss Convergece ')

plt.title('Convergence of CAE on Optimal Parameters - MSE Loss Function')
...
np.savetxt('LSTM_MSE.txt', [meanLosses])
...
testLSTM.load_state_dict(torch.load('bestLSTM_BCE.pth'))
...
with torch.no_grad():
    ...
    for data in loadTest:
        ...
        for i in range(len(video)):
            ...
            prediction = testLSTM.forward(basisCompressed)
            ...
            individualLosses = criterion(predictionFinal, targetFinal)
            videoWiseLosses.append(individualLosses)

            videoID.append(i + 41)

        test_loss = criterion(allPred, allTarg)
...
# Video Wise Individual Losses MSE Metric
plt.figure(figsize = (10, 10))
plt.bar(videoID, videoWiseLosses, color = 'black')
plt.title('Video Wise Losses - MSE Metric - LSTM prediction')
plt.xlabel('Video ID')
plt.ylabel('Losses (%)')
plt.grid()
