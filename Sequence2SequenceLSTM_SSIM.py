...
for epoch in range(epochs):
    ...
    for data in loadTrain:
        ...
        loss = ms_ssim_loss + critMSE(allPred, allTargs)
    ...
    with torch.no_grad():
        ...
        for data in loadValid:
            ...
            val_loss = ms_ssim_loss_val + critMSE(allPred, allTargs)
            cumuLoss += val_loss
...
torch.save(best_model_state, 'bestLSTM_SSIM+MSE.pth')
...
np.savetxt('LSTM_SSIM+MSE.txt', [meanLosses])
...
testLSTM.load_state_dict(torch.load('bestLSTM_SSIM+BCE.pth'))
...
with torch.no_grad():
    ...
    for data in loadTest:
        ...
        for i in range(len(video)):
            ...
            individualLosses = 1 - criterion(predictionFinal, targetFinal, win_size=7) + critMSE(predictionFinal, targetFinal)
            videoWiseLosses.append(individualLosses)

            videoID.append(i + 41)

        test_loss = (1 - criterion(allPred, allTarg, win_size=7)) + critMSE(allPred, allTarg)
...
plt.title('Video Wise Losses - MSE+SSIM Metric - LSTM prediction')
plt.xlabel('Video ID')
plt.ylabel('Losses (%)')
plt.grid()
